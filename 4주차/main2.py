...
while True:
    ...
    for det in dets:
        ...
        # for i, point in enumerate(shape.parts()):#shape.parts()에 점이 저장되어있음 enumerate는 인덱스와 인자 반환
        #     cv2.circle(img, center=(point.x, point.y), radius=2, color=(0, 0, 255), thickness=-1)
        #     cv2.putText(img, text=str(i), org=(point.x, point.y), fontFace=cv2.FONT_HERSHEY_SIMPLEX, fontScale=0.8,
        ...
        glasses_h = int(h / w * glasses_w)#비율을 유지하면서 변환
